...
public:
	...
	BaryCoord operator*(const double scalar) const {
        return {u * scalar, v * scalar, w * scalar};
    }
	BaryCoord operator+(const BaryCoord& bc) const {
        return {u + bc.u, v + bc.v, w + bc.w};
    }
...
public:
	...
	Vector3d blossomBiquadBezier(const std::array<Vector3d, 6>& a, const BaryCoord& coord0, const BaryCoord& coord1) {
	...
	Vector3d blossomBicubicBezier(const std::array<Vector3d, 10>& p, const BaryCoord& coord0, const BaryCoord& coord1, const BaryCoord& coord2) {
	...
	Vector3d blossomBicubicBezier(const BaryCoord& coord) {
